    ...
    def handle(self, method: str, path: str,
        ...
        try:
            if method == "POST" and path == "/books":
                ...
                output = self._create.execute(dto)
                return {"status": 201, "body": output.to_dict()}
            elif method == "GET" and path == "/books":
                outputs = self._list.execute()
                return {"status": 200, "body": [r.to_dict() for r in outputs]}
            elif method == "GET" and path.startswith("/books/"):
                book_id = int(path.split("/")[-1])
                output = self._get.execute(book_id)
                return {"status": 200, "body": output.to_dict()}
            elif method == "PUT" and path.startswith("/books/"):
                book_id = int(path.split("/")[-1])
                dto = UpdateBookDTO(**data)
                output = self._update.execute(book_id, dto)
                return {"status": 200, "body": output.to_dict()}
